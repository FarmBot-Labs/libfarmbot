...
ALL := format $(LIBFARMBOT_OUT_DIR) $(LIBFARMBOT) $(LIBFARMBOT_TEST)
CLEAN :=
PHONY := format all clean test
...
test: $(LIBFARMBOT) $(LIBFARMBOT_TEST)
	LD_LIBRARY_PATH=$(LIBFARMBOT_OUT_DIR) gdb test/login_test.test

format:
	astyle -n -r "$(LIBFARMBOT_SRC_DIR)/*.c" "$(LIBFARMBOT_INCLUDE_DIR)/*.h"
