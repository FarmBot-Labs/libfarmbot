...
Farmbot* farmbot_init() {
    ...
    farmbot->connected = false;
    farmbot->attempts = 0;

    debug_print("Farmbot(%p) init: %d\r\n", farmbot, farmbot->connected);
    return farmbot;
}
...
FarmbotJWT* farmbot_login(Farmbot* farmbot, char* email, char* password, char* server) {
    ...
    size_t server_length, server_size;
    server_length = strlen(server);
    server_size   = server_length * sizeof(char);
    if(farmbot->attempts == 0) {
        farmbot->server = malloc(server_size);
    } else {
        farmbot->server = realloc(farmbot->server, server_size);
    }

    farmbot->attempts++;

    if(farmbot->server == NULL)
        return NULL;

    strcpy(farmbot->server, server);
